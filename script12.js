...
function filterProducts(items, criteria) {
    return items.filter(item => {
        return Object.keys(criteria).every(key => item[key] === criteria[key]);
    });
}
const italianItems = filterProducts(items, { country: "Italy" });
console.log(italianItems);

const coffeeItems = filterProducts(items, { category: "Coffee" });
